...
const startPath = "/loop-designer/start/loop-design";
...
export async function GET(request: Request) {
  ...
  const session = await createSession(user, { workflow: "loop_design" });
  return NextResponse.redirect(new URL(`/loop-designer/sessions/${session.id}`, requestOrigin), 303);
}
