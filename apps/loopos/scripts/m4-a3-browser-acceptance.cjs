...
const adminDatabaseUrl = process.env.M4A3_ADMIN_DATABASE_URL ?? "postgresql://heyiqing@localhost:5432/postgres";
const port = Number(process.env.M4A3_BROWSER_PORT ?? 3225);
const baseUrl = `http://127.0.0.1:${port}`;
const evidenceDir = process.env.M4A3_EVIDENCE_DIR ?? `/tmp/loopos-m4a3-browser-${Date.now()}`;
const password = "M4A3-browser-pass-2026";
const suffix = `${Date.now()}_${randomBytes(3).toString("hex")}`;
const database = `loopos_m4a3_browser_${suffix}`;
...
const ids = {
  org: "org-m4a3",
  ownerUser: "user-m4a3-owner",
  otherUser: "user-m4a3-other",
  ownerMembership: "membership-m4a3-owner",
  otherMembership: "membership-m4a3-other",
  ownerCircle: "circle-m4a3-owner",
  otherCircle: "circle-m4a3-other",
  ownerPerson: "person-m4a3-owner",
  otherPerson: "person-m4a3-other",
  ownerRole: "role-m4a3-owner",
  meeting: "meeting-m4a3",
  cycle: "cycle-m4a3",
  proposal: "goal-proposal-m4a3",
  proposalTarget: "proposal-target-m4a3",
  decision: "goal-decision-m4a3",
  goal: "goal-m4a3",
  target: "target-m4a3",
};

const accounts = {
  owner: { email: `m4a3-owner-${suffix}@loopos.test`, name: "M4A3 Owner" },
  other: { email: `m4a3-other-${suffix}@loopos.test`, name: "M4A3 Other" },
};
...
async function seedFixture(connectionString) {
  ...
  await withClient(connectionString, async (client) => {
    ...
    await client.query(`INSERT INTO "organizations" ("id", "name", "slug", "createdAt", "updatedAt")
      VALUES ($1, 'M4A3 Browser Org', $2, TIMESTAMP '${now}', TIMESTAMP '${now}')`, [
      ids.org,
      `m4a3-browser-${suffix}`,
    ]);
    ...
    await client.query(`INSERT INTO "circles" ("id", "organizationId", "name", "number", "type", "purpose", "parentId", "createdAt", "updatedAt")
      VALUES ($1, $2, 'M4A3 目标回路', 'CUSTOM', 'PRODUCTION', 'Private brief browser evidence', NULL, TIMESTAMP '${now}', TIMESTAMP '${now}'),
             ($3, $2, 'M4A3 旁观回路', 'CUSTOM', 'PRODUCTION', 'No private goal access', $1, TIMESTAMP '${now}', TIMESTAMP '${now}')`, [
      ids.ownerCircle,
      ids.org,
      ids.otherCircle,
    ]);
    ...
    await client.query(`INSERT INTO "role_defs" ("id", "organizationId", "name", "purpose", "accountabilities", "category", "status", "circleId", "createdAt", "updatedAt")
      VALUES ($1, $2, 'M4A3 Goal Owner', 'Own browser evidence goal', 'Maintain private brief evidence', 'OPERATIONS', 'ACTIVE', $3, TIMESTAMP '${now}', TIMESTAMP '${now}')`, [
      ids.ownerRole,
      ids.org,
      ids.ownerCircle,
    ]);
    ...
    await client.query(`INSERT INTO "meetings" ("id", "organizationId", "title", "type", "agenda", "notes", "notesRevision", "durationMin", "startedAt", "circleId", "createdAt")
      VALUES ($1, $2, 'M4A3 Goal Adoption', 'TACTICAL', '[]', 'Adopt M4A3 goal', 0, 30, TIMESTAMP '${now}', $3, TIMESTAMP '${now}')`, [
      ids.meeting,
      ids.org,
      ids.ownerCircle,
    ]);
    ...
    await client.query(`INSERT INTO "goal_cycles" ("id", "organizationId", "name", "status", "startAt", "endAt", "checkInCadenceDays", "createdAt", "updatedAt")
      VALUES ($1, $2, 'M4A3 Cycle', 'PLANNED', TIMESTAMP '2026-07-01 00:00:00', TIMESTAMP '2026-09-30 00:00:00', 7, TIMESTAMP '${now}', TIMESTAMP '${now}')`, [
      ids.cycle,
      ids.org,
    ]);
    ...
    await client.query(`INSERT INTO "goal_proposal_revisions" ("organizationId", "proposalId", "revision", "title", "intendedOutcome", "ownerRoleId", "authoredById", "createdAt")
      VALUES ($1, $2, 1, 'M4A3 私人简报目标', 'Browser can inspect private drift signals', $3, $4, TIMESTAMP '2026-07-01 01:10:00')`, [
      ids.org,
      ids.proposal,
      ids.ownerRole,
      ids.ownerPerson,
    ...
    await client.query(`INSERT INTO "goal_proposal_targets" ("id", "organizationId", "proposalId", "revision", "position", "label", "kind", "acceptanceCriteria", "createdAt")
      VALUES ($1, $2, $3, 1, 1, 'M4A3 Evidence Target', 'MILESTONE', 'Browser evidence exists', TIMESTAMP '2026-07-01 01:20:00')`, [
      ids.proposalTarget,
      ids.org,
      ids.proposal,
    ]);
    ...
    await client.query(`INSERT INTO "goal_decisions" ("id", "organizationId", "proposalId", "revision", "outcome", "meetingId", "recorderId", "mutationKey", "decidedAt")
      VALUES ($1, $2, $3, 1, 'ADOPTED', $4, $5, 'm4a3-goal-adopt', TIMESTAMP '2026-07-01 01:30:00')`, [
      ids.decision,
      ids.org,
      ids.proposal,
      ids.meeting,
    ...
    await client.query(`INSERT INTO "goals" ("id", "organizationId", "cycleId", "circleId", "title", "intendedOutcome", "ownerRoleId", "status", "adoptedDecisionId", "createdAt")
      VALUES ($1, $2, $3, $4, 'M4A3 私人简报目标', 'Browser can inspect private drift signals', $5, 'ACTIVE', $6, TIMESTAMP '2026-07-01 02:00:00')`, [
      ids.goal,
      ids.org,
      ids.cycle,
      ids.ownerCircle,
    ...
    await client.query(`INSERT INTO "goal_targets" ("id", "organizationId", "goalId", "sourceProposalTargetId", "position", "label", "kind", "acceptanceCriteria", "createdAt")
      VALUES ($1, $2, $3, $4, 1, 'M4A3 Evidence Target', 'MILESTONE', 'Browser evidence exists', TIMESTAMP '2026-07-01 02:10:00')`, [
      ids.target,
      ids.org,
      ids.goal,
      ids.proposalTarget,
...
async function runBrowser(connectionString) {
  ...
  try {
    ...
    await owner.getByText("目标需要更新证据：M4A3 私人简报目标").waitFor();
    await owner.getByText("这个活跃目标还没有可用的进展证据。").waitFor();
    await owner.screenshot({ path: `${evidenceDir}/01-owner-private-brief.png`, fullPage: true });

    await Promise.all([
      owner.waitForURL(new RegExp(`${baseUrl.replaceAll(".", "\\.")}/app/goals\\?cycle=`), { timeout: 30_000 }),
      owner.getByLabel("私人简报").getByRole("link", { name: /M4A3 私人简报目标/ }).first().click(),
    ]);
    await owner.screenshot({ path: `${evidenceDir}/02-owner-source-navigation.png`, fullPage: true });
    ...
    if (await other.getByText("M4A3 私人简报目标").count()) {
    ...
    writeFileSync(`${evidenceDir}/db-counts.json`, JSON.stringify({ beforeCounts, afterCounts }, null, 2));
    ...
    if (JSON.stringify(beforeCounts) !== JSON.stringify(afterCounts)) {
      throw new Error(`Private brief browser path mutated data: ${JSON.stringify({ beforeCounts, afterCounts })}`);
    }
