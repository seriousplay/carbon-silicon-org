...
async function seedReferenceData(databaseUrl, email, suffix) {
  ...
  try {
    ...
    if (!row) throw new Error("M3-D fixture person not found after registration");
    const organizationId = row.organizationId;
    const sourceCircleId = `m3d-source-${suffix}`;
    const targetCircleId = `m3d-target-${suffix}`;
    const roleId = `m3d-role-${suffix}`;
    const interfaceId = `m3d-interface-${suffix}`;
    await client.query(`
...
async function fetchCounts(databaseUrl, organizationId) {
  ...
  try {
    const result = await pool.query(`
      SELECT
        (SELECT count(*)::int FROM business_loops WHERE "organizationId" = $1) AS loops,
        (SELECT count(*)::int FROM business_loop_versions WHERE "organizationId" = $1) AS versions,
...
async function main() {
  ...
  const screenshotDir = option("--screenshot-dir", `/tmp/loopos-m3d-${Date.now()}`);
  const databaseUrl = process.env.DATABASE_URL;
  if (!databaseUrl) throw new Error("DATABASE_URL is required");
  mkdirSync(screenshotDir, { recursive: true });

  const suffix = String(Date.now());
  const email = `m3d-admin-${suffix}@loopos.test`;
  const password = `M3d-admin-${suffix}!`;
  const orgName = `M3-D Admin ${suffix}`;
  ...
  try {
    ...
    await page.getByLabel("你的姓名").fill("M3-D Admin");
    ...
    await page.getByRole("heading", { name: "业务回路草稿" }).waitFor();
    await page.locator('input[name="name"]').fill("客户需求到模型能力草稿");
    await page.locator('input[name="purpose"]').fill("把客户需求、数据流动和模型交付串成一个运营闭环。");
    await page.getByRole("button", { name: "新建草稿" }).click();
    await page.getByText("业务回路草稿已创建").waitFor();
    await page.reload({ waitUntil: "networkidle" });

    await page.locator('input[name="activityName"]').fill("确认高价值需求");
    await page.locator('select[name="activityType"]').selectOption("WORK");
    ...
    await page.getByText("客户需求到模型能力草稿").waitFor();
    ...
    if (stats.join(",") !== "3,1,1,1") {
      throw new Error(`unexpected authoring stats: ${stats.join(",")}`);
    }
    const counts = await fetchCounts(databaseUrl, references.organizationId);
    if (counts.loops !== 1 || counts.versions !== 1 || counts.activities !== 1 || counts.edges !== 1 || counts.evidence !== 1) {
      throw new Error(`unexpected persisted counts: ${JSON.stringify(counts)}`);
    }
    ...
    await page.screenshot({ path: `${screenshotDir}/desktop-authoring.png`, fullPage: true });
    await page.close();
